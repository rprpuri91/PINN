...
=======
        V_train = self.velocity_cartesian_vjp(X_star)
        V_test = self.velocity_cartesian_vjp(X)
        X_in_train = torch.from_numpy(X_star).float().to(self.device)
...
=======
        phi, phi_x_y = torch.autograd.functional.vjp(self.potential_fn, g, v, create_graph=True)
>>>>>>> 9c150e5ac03cf7481d2b3b3dd99f469ccc28c54c
...
=======
    ...
    def loss_total(self):
        ...
        X_initial, X_bc, X_outlet, X_in, X_cyl_bc, indices = preprocessing()
        ...
        loss_cyl_bc = self.loss(X_cyl_bc)

        total_loss = loss_initial + loss_bc + loss_cyl_bc + loss_outlet
>>>>>>> 9c150e5ac03cf7481d2b3b3dd99f469ccc28c54c
...
=======
    x_values = np.arange(-8.0, 8.0, 0.3).tolist()
    y_values = np.arange(-4.0, 4.0, 0.3).tolist()
>>>>>>> 9c150e5ac03cf7481d2b3b3dd99f469ccc28c54c
    x_values.append(8.0)
    y_values.append(4.0)
...
=======
        elif radius < (R+0.03) and radius > (R-0.03):
            indices_R.append(i)


    indices = indices_R + indices_inR

    X_domain = np.delete(X_in, indices, axis=0)

    X_cyl_bc = np.take(X_in, indices_R, axis=0)

    #print("domain",X_in.shape)

    return X_initial, X_bc, X_outlet, X_domain, X_cyl_bc, indices
>>>>>>> 9c150e5ac03cf7481d2b3b3dd99f469ccc28c54c
...
=======
layers = np.array([2, 80, 80, 80, 80, 2])

nu = 0.8

epochs = 500

X_initial, X_bc, X_outlet, X_domain, X_cyl_bc, indices = preprocessing()
>>>>>>> 9c150e5ac03cf7481d2b3b3dd99f469ccc28c54c
...
=======
X_in_train, X_test, V_train, V_test = model.training_test_data(X_domain)
...
f.close()
=======
error_vec, V_pred = model.test(model, X_test, V_test)

result = [V_pred, V_test, model.error, model.trainingloss, indices]
f = open('result_potential_flow.pkl', 'wb')
pickle.dump(result, f)
f.close()
